...
static size_t gf2_rank(vector<vector<bool>> rows)
{
    ...
    {
        size_t pivot = rank;
        while (pivot < rows.size() && !rows[pivot][col])
            ++pivot;
        if (pivot == rows.size())
            continue;
...
public:
    ...
    int test_on_example_circuit()
    {
        try
        {
            ...
            for (const auto &c : base)
            {
                if (cycle_is(c, tri124))
                    found124 = true;
                if (cycle_is(c, tri234))
                    found234 = true;
            }
...
int main()
{
    ...
    if (t.test_returns_correct_cycle_count() != EXIT_SUCCESS)
        return EXIT_FAILURE;
    cout << "test_returns_correct_cycle_count" << endl;
    if (t.test_cycles_are_linearly_independent() != EXIT_SUCCESS)
        return EXIT_FAILURE;
    cout << "test_cycles_are_linearly_independent" << endl;
    if (t.test_finds_minimal_cycle_basis() != EXIT_SUCCESS)
        return EXIT_FAILURE;
    cout << "test_finds_minimal_cycle_basis" << endl;
    if (t.test_single_cycle_graph() != EXIT_SUCCESS)
        return EXIT_FAILURE;
    cout << "test_single_cycle_graph" << endl;
    if (t.test_tree_graph_has_empty_basis() != EXIT_SUCCESS)
        return EXIT_FAILURE;
    cout << "test_tree_graph_has_empty_basis" << endl;
    if (t.test_on_example_circuit() != EXIT_SUCCESS)
        return EXIT_FAILURE;
